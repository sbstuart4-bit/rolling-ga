...
export function ArtistGuidedDemoMobileChrome(props: ArtistGuidedDemoChromeProps) {
  ...
  return (
    <>
      <div
        className="fixed inset-x-0 bottom-0 z-40 border-t border-border bg-[#0f0f10]/95 backdrop-blur-lg xl:hidden"
        aria-label="Artist guided demo controls"
      ...
      >
        ...
        <div className="border-t border-border/60 px-3 pb-2 pt-1">
          <ArtistGuidedDemoStepNav session={session} step={step} totalSteps={totalSteps} compact />
        </div>
      </div>
      ...
      <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
        <SheetContent
          side="bottom"
          className="max-h-[min(85dvh,640px)] overflow-y-auto rounded-t-2xl border-border bg-[#0f0f10] pb-[calc(1rem+env(safe-area-inset-bottom,0px))] xl:hidden"
        >
          <SheetHeader className="border-b border-border/60 pb-3 text-left">
            ...
            <SheetDescription className="text-xs uppercase tracking-wider">
              Artist guided demo · Step {step.step} of {totalSteps}
            </SheetDescription>
          </SheetHeader>
          ...
          <div className="border-t border-border/60 px-4 py-3">
            <ArtistGuidedDemoExitForm compact />
          </div>
        </SheetContent>
      </Sheet>
    </>
